...
| `WITH` (CTEs), window functions (`OVER`) | `OFFSET`, `FETCH FIRST`, `UNION` / `INTERSECT` / `EXCEPT [ALL]`, `RIGHT` / `FULL` / `NATURAL` joins and `USING` **are supported** ([reference](sql.md#reading)); write the CTE as a derived table for now |
