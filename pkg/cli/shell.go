...
Statements: CREATE DATABASE, DROP DATABASE [CASCADE], SHOW DATABASES, USE db,
  ...
  CREATE / DROP INDEX, ALTER INDEX ... RENAME TO, ALTER TABLE ... ADD/DROP
  COLUMN | RENAME TO | RENAME [COLUMN] | RENAME CONSTRAINT | ALTER COLUMN SET /
  DROP DEFAULT | SET (retention | shards), DROP TABLE, TRUNCATE [RESTART
  IDENTITY] [CASCADE] (IF [NOT] EXISTS on every CREATE / DROP / ALTER),
  CREATE [OR REPLACE] VIEW, DROP VIEW [CASCADE], SHOW VIEWS, CREATE TABLE ...
  AS SELECT / (LIKE t INCLUDING ALL), ALTER COLUMN ... TYPE, COMMENT ON,
  INSERT [ON CONFLICT ...] / UPSERT, SELECT [... AS OF SYSTEM
