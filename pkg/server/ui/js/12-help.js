...
const HELP = {
  ...
  "refused before verification": "Sign-in attempts this node turned away before it checked the password at all, since it started. Verifying a password costs real processor time, so the node bounds what a caller can spend on it: a caller asking too often is rate-limited before its password is checked; a caller having passwords checked too often — a valid credential re-sent on every request, which costs the node the same work whether or not it is right — is refused once its verification budget is spent, and succeeding does not refill that budget; and once the node is already checking as many passwords at once as it allows, the rest are refused until one finishes. A number climbing steadily means something is authenticating far faster than any client should — the problems panel reports the rate, because a lifetime total stays non-zero forever after one bad afternoon. It counts what was refused, which is not the same as what was attempted. The rate limit covers the SQL port as well as the sign-in form and HTTP Basic, on one budget; the verification budget and cap cover the HTTP doors, since a SQL password exchange costs the node no verification to bound.",
