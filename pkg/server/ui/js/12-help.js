...
const HELP = {
  ...
  "refused before verification": "Sign-in attempts this node turned away before it checked the password at all, since it started. Verifying a password costs real processor time, so the node bounds what a caller can spend on it, and the figure's qualifier says which bound refused. Rate-limited: the caller was asking too often, and its password was not checked. Over the verification budget: the caller was having passwords checked too often — a valid credential re-sent on every request costs the node the same work whether or not it is right, so succeeding does not refill this budget. At the concurrent-verification cap: the node was already checking as many passwords at once as it allows, and the rest were refused until one finished. A number climbing steadily means something is authenticating far faster than any client should — the problems panel reports the rate, because a lifetime total stays non-zero forever after one bad afternoon. It counts what was refused, not what was attempted: a flood that stays inside the bounds still costs the node the work and appears here as nothing. The rate limit covers the SQL port as well as the sign-in form and HTTP Basic, on one budget; the verification budget and cap cover the HTTP doors, since a SQL password exchange costs the node no verification to bound.",
