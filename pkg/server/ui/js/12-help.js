...
const HELP = {
  ...
  "refused before verification": "Sign-in attempts this node turned away before it checked the password at all, since it started. Verifying a password costs real processor time, so the node bounds what an unauthenticated caller can spend: a caller asking too often is rate-limited, and once the node is already checking as many passwords at once as it allows, the rest are refused until one finishes. A number climbing steadily means something is attempting to authenticate far faster than any client should — the problems panel reports the rate, because a lifetime total stays non-zero forever after one bad afternoon. It counts what was refused, which is not the same as what was attempted: a caller holding any valid password is let through and restores its own budget each time it succeeds, so a flood of successful sign-ins costs this node the same work and is counted here as nothing at all. It also covers the HTTP doors only — the sign-in form and HTTP Basic. The SQL port has no limiter of its own, so password guessing against it is not refused, not counted here, and will leave this reading none.",
